...
#pragma region RichEditImageOle
	...
	BOOL RichEditImageOle::SetImageSkin(ISkinObj* pSkin)
	{
		...
		_oleView.SetDelayDraw(_pImageView->GetFrameCount() > 1); // ����1֡��ͼƬ��Ҫ�ӳ�ˢ��
	...
	HRESULT RichEditImageOle::InternalDraw(IRenderTarget* pRt, IRegionS* prgn, DWORD cp)
	{
		CRect rgnBoxRect;
...
#pragma region RichEditAudioOle
	...
	{
		BOOL ret = RichEditOleBase::InitOleWindow(pHost);
		int nAudioDuration = std::atoi(S_CW2A(_audio_duration).c_str());
	...
	void RichEditAudioOle::UpdatePosition()
	{
		if (_spAdviseSink)
		{
			...
			_oleView.SetOleWindowRect(CRect(0, 0, 0, 0)); // �Ѿ�ʧЧ
			_oleView.Move2(0, 0, _sizeNatural.cx, _sizeNatural.cy);
			CalculateExtentSize(_sizeNatural);
			_spAdviseSink->OnViewChange(DVASPECT_CONTENT, -1);
		}
...
#pragma region RichEditVideoOle
	...
	BOOL RichEditVideoOle::InitOleWindow(IRichEditObjHost* pHost)
	{
		...
		if (pVideoWnd)
		{
			...
			IBitmapS* pFrameImg = SResLoadFromFile::LoadImage(_video_cover_path);
		...
		if (L"unknow" == _video_duration)
			sstrVideoDuration = L"δ֪";
		else
		{
			int nVideoDuration = atoi(S_CW2A(_video_duration).c_str());
	...
	void RichEditVideoOle::UpdatePosition()
	{
		if (_spAdviseSink)
		{
			...
			{
				IBitmapS* pFrameImg = SResLoadFromFile::LoadImage(_video_cover_path);
				nWidth = pFrameImg->Width();
				nHeight = pFrameImg->Height();
			}
			else
			{
				nWidth = std::stoi(_video_cover_width.c_str());
				nHeight = std::stoi(_video_cover_height.c_str());
			}
			SIZE tempSize;
			tempSize.cx = nWidth;
			tempSize.cy = nHeight;
...
#pragma region RichEditWebOle
	...
	BOOL RichEditLinkOle::InitOleWindow(IRichEditObjHost* pHost)
	{
		...
		IBitmapS* pImg = SResLoadFromFile::LoadImage(_link_imgpath);
		if (pUrlImg && pImg)
		{
...
#pragma region RichEditLocationOle
	...
	{
		BOOL bRet = RichEditOleBase::InitOleWindow(pHost);
		IBitmapS* pLocationImg = SResLoadFromFile::LoadImage(_location_imgpath);
	...
	{
		IBitmapS* pLocationImg = SResLoadFromFile::LoadImage(sstrImagePath);
		SImageWnd* pLocationImgWnd = _oleView.FindChildByName2<SImageWnd>(L"location_img");
		if (pLocationImg && pLocationImgWnd)
		{
			pLocationImgWnd->SetImage(pLocationImg);
	...
	void RichEditLocationOle::UpdatePosition()
	{
		if (_spAdviseSink)
		{
			int nWidth = std::stoi(_img_width.c_str());
			int nHeight = std::stoi(_img_height.c_str());

			SIZE tempSize;
			tempSize.cx = nWidth;
			tempSize.cy = nHeight + 150;
	...
	{
		_img_width = SStringW().Format(L"%d", nWidth);
		_img_height = SStringW().Format(L"%d", nHeight);
		UpdatePosition();
	}
#pragma endregion
...
#pragma region RichSeparatorBar
...
#pragma region RichEditReminderOle
	...
	BOOL RichEditReminderOle::InitFromXml(IXmlNode* pNode)
	{
	...
	void RichEditReminderOle::CalculateNatureSize()
	{
		...
		pRT->SelectObject(SFontPool::GetFont(_font, GetScale()));
...
#pragma region RichEditRevokeOle
